...
class DVCEval(object):
    def __init__(
        self, ground_truth, prediction, tious=None, max_proposals=1000, sentsim=None
    ):
        self.tious = tious
                ...
                if len(res[vid_id]) == 0 or len(gts[vid_id]) == 0:
                    ...
                    if isinstance(method, list):
                        score, scores = scorer.compute_score(
                            gts[vid_id], res[vid_id], verbose=0
                        )
                    else:
...
def dvc_format(caption):
    ...
    for i in range(len(timestamps)):
        assert (
            isinstance(timestamps[i], list)
            and len(timestamps[i]) == 2
            and isinstance(timestamps[i][0], (int, float))
            and isinstance(timestamps[i][1], (int, float))
        )
...
def dvc_eval(samples, st):
    ...
    for sample in samples:
        ...
        pred["results"][sample["video"]] = [
            dict(sentence=c, timestamp=t) for t, c in zip(time, cap)
        ]
    ...
    if gt_dict:
        evaluator = DVCEval(
            ground_truth=gt_dict, prediction=pred, tious=iou_thr, sentsim=st
        )
        evaluator.evaluate()
